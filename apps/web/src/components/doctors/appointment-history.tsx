...
import { useDoctorAppointments } from '@/hooks/use-doctors';
...
export function AppointmentHistory({ doctorId }: { doctorId: string }) {
  const [page, setPage] = useState(1);
  const { data, isLoading } = useDoctorAppointments(doctorId, page);

  if (isLoading) {
    return <p className="text-sm text-muted-foreground py-8 text-center">Loading appointments...</p>;
  }
